...
procedure TPK3Writer.SaveToFile(const filename: string);
var
...
procedure TPK3Writer.SaveToStream(ZipFileStream: TStream);
var
  i: integer;
begin
...
function TPK3Writer.GetUncompressed(i: integer): string;
var
...
begin
  if (i < 0) or (i > High(Files)) then
    raise Exception.Create(
      'TPK3Writer.GetUncompressed(): Index out of range.');
  ...
  if LoadedCRC32 <> Files[i].CommonFileHeader.Crc32 then
    raise EZipFileCRCError.CreateFmt(
      'TPK3Writer.GetUncompressed(): CRC Error in "%s".', [Files[i].filename]);
end;

procedure TPK3Writer.SetUncompressed(i: integer; const Value: string);
var
...
begin
  if i > High(Files) then
    raise Exception.Create(
      'TPK3Writer.SetUncompressed(): Index out of range.');
...
function TPK3Writer.AddFile(const name: string; FAttribute: LongWord = 0): integer;
begin
...
function TPK3Writer.GetDateTime(i: integer): TDateTime;
begin
  if i > High(Files) then
    raise Exception.Create(
      'TPK3Writer.GetDateTime(): Index out of range.');
  Result := FileDateToDateTime(Files[i].CommonFileHeader.LastModFileTimeDate);
end;

procedure TPK3Writer.SetDateTime(i: integer; const Value: TDateTime);
begin
  if i > High(Files) then
    raise Exception.Create(
      'TPK3Writer.SetDateTime(): Index out of range.');
  Files[i].CommonFileHeader.LastModFileTimeDate := DateTimeToFileDate(Value);
end;

function TPK3Writer.GetCount: integer;
begin
  Result := High(Files) + 1;
end;

function TPK3Writer.GetName(i: integer): string;
begin
  Result := Files[i].filename;
end;

procedure TPK3Writer.SetName(i: integer; const Value: string);
begin
