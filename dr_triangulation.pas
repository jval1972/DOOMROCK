unit dr_triangulation;

(*****************************************************************************
**                                                                           *
** The TriangulateDelaunayClipping function needs and uses the Clipper       *
** library from http://www.angusj.com/delphi/clipper.php for the polygon     *
** clipping work                                                             *
**                                                                           *
******************************************************************************
...
{$undef STANDALONE}
// For only to have the Triangulate variant without any unit dependencies except the RTL itself, of course

interface

type
  PTriangulationPoint = ^TTriangulationPoint;

  TTriangulationPoint = record
    x, y: double;
  end;

  TTriangulationPolygon = array of TTriangulationPoint;

  TTriangulationPolygons = array of TTriangulationPolygon;

  PTriangulationTriangle = ^TTriangulationTriangle;
  TTriangulationTriangle = array[0..2] of TTriangulationPoint;

  TTriangulationTriangles = array of TTriangulationTriangle;

  TTriangulationPolygonFillRule = (btpfrEVENODD, btpfrNONZERO,
    btpfrPOSITIVE, btpfrNEGATIVE, btpfrABSGEQTWO);
...
function Triangulate(const InputPolygons: TTriangulationPolygons;
  var OutputTriangles: TTriangulationTriangles;
  InputPolygonFillRule: TTriangulationPolygonFillRule = btpfrEVENODD;
  Quality: longint = $7fffffff): boolean;
...
function TriangulateDelaunayClipping(const InputPolygons: TTriangulationPolygons;
  var OutputTriangles: TTriangulationTriangles;
  InputPolygonFillRule: TTriangulationPolygonFillRule = btpfrEVENODD): boolean;
{$endif}
...
function TriangulateSeidel(const InputPolygons: TTriangulationPolygons;
  var OutputTriangles: TTriangulationTriangles): boolean;
{$endif}
...
function TriangulateEarClipping(const InputPolygon: TTriangulationPolygon;
  var OutputTriangles: TTriangulationTriangles): boolean;
...
const
  ...
  BOTTOMLEFT = 3;
type
  PGarbageCollectedHeader = ^TGarbageCollectedHeader;
...
var
  ...
  var
    Triangle: PTriangulationTriangle;
  begin
    if length(OutputTriangles) <= CountOutputTriangles then
    begin
      SetLength(OutputTriangles, (CountOutputTriangles + 1) * 2);
    end;
    Triangle := @OutputTriangles[CountOutputTriangles];
    Inc(CountOutputTriangles);
    if (((bx - ax) * (by + ay)) + ((cx - bx) * (cy + by)) + ((ax - cx) * (ay + cy))) < 0.0 then
    begin
      Triangle^[0].x := ax;
      Triangle^[0].y := ay;
      Triangle^[1].x := bx;
  ...
  begin
    ...
    if assigned(GarbageCollector.Last) then
    begin
  ...
  begin
    if assigned(PGarbageCollectedHeader(Data)^.Previous) then
    begin
      PGarbageCollectedHeader(Data)^.Previous^.Next := PGarbageCollectedHeader(Data)^.Next;
    end
    else if GarbageCollector.First = Data then
    begin
      GarbageCollector.First := PGarbageCollectedHeader(Data)^.Next;
    end;
    if assigned(PGarbageCollectedHeader(Data)^.Next) then
    begin
      PGarbageCollectedHeader(Data)^.Next^.Previous :=
        PGarbageCollectedHeader(Data)^.Previous;
    end
    else if GarbageCollector.Last = Data then
    begin
  ...
  begin
    ...
    for i := 0 to Result^.NodeCapacity - 2 do
    begin
  ...
  var
    ...
    i: longint;
  begin
    if DynamicAABBTree^.FreeList = daabbtNULLNODE then
    begin
      Inc(DynamicAABBTree^.NodeCapacity, DynamicAABBTree^.NodeCapacity);
      ReallocMem(DynamicAABBTree^.Nodes, DynamicAABBTree^.NodeCapacity *
        SizeOf(TDynamicAABBTreeNode));
      FillChar(DynamicAABBTree^.Nodes^[DynamicAABBTree^.NodeCount],
        (DynamicAABBTree^.NodeCapacity - DynamicAABBTree^.NodeCount) * SizeOf(
        TDynamicAABBTreeNode), #0);
      for i := DynamicAABBTree^.NodeCount to DynamicAABBTree^.NodeCapacity - 2 do
      begin
  ...
  function DynamicAABBTreeBalance(const DynamicAABBTree: PDynamicAABBTree;
    NodeAID: longint): longint;
  var
    NodeA, NodeB, NodeC, NodeD, NodeE, NodeF, NodeG: PDynamicAABBTreeNode;
    NodeBID, NodeCID, NodeDID, NodeEID, NodeFID, NodeGID, NodeBalance: longint;
  begin
    NodeA := @DynamicAABBTree^.Nodes^[NodeAID];
    if (NodeA.Children[0] < 0) or (NodeA^.Height < 2) then
    begin
      Result := NodeAID;
    end
    else
    begin
      ...
      begin
        ...
        if NodeC^.Parent >= 0 then
        begin
          if DynamicAABBTree^.Nodes^[NodeC^.Parent].Children[0] = NodeAID then
          begin
      ...
      begin
        ...
        if NodeB^.Parent >= 0 then
        begin
          if DynamicAABBTree^.Nodes^[NodeB^.Parent].Children[0] = NodeAID then
          begin
  ...
  begin
    ...
    if DynamicAABBTree^.Root < 0 then
    begin
    ...
    begin
      ...
      while DynamicAABBTree^.Nodes^[Index].Children[0] >= 0 do
      begin
        ...
        if DynamicAABBTree^.Nodes^[Children[0]].Children[0] < 0 then
        begin
          Costs[0] := AABBCost(AABB) + InheritanceCost;
        end
        else
        begin
          Costs[0] := (AABBCost(AABB) - AABBCost(DynamicAABBTree^.Nodes^[Children[0]].AABB)) +
            InheritanceCost;
        end;
        AABB := AABBCombine(LeafAABB, DynamicAABBTree^.Nodes^[Children[1]].AABB);
        if DynamicAABBTree^.Nodes^[Children[1]].Children[1] < 0 then
        begin
          Costs[1] := AABBCost(AABB) + InheritanceCost;
        end
        else
        begin
          Costs[1] := (AABBCost(AABB) - AABBCost(DynamicAABBTree^.Nodes^[Children[1]].AABB)) +
            InheritanceCost;
        end;
        if (Cost < Costs[0]) and (Cost < Costs[1]) then
        begin
          break;
        end
        else
        begin
          if Costs[0] < Costs[1] then
          begin
            Index := Children[0];
          end
          else
      ...
      DynamicAABBTree^.Nodes^[NewParent].AABB :=
        AABBCombine(LeafAABB, DynamicAABBTree^.Nodes^[Sibling].AABB);
      DynamicAABBTree^.Nodes^[NewParent].Height := DynamicAABBTree^.Nodes^[Sibling].Height + 1;
      if OldParent >= 0 then
      begin
        if DynamicAABBTree^.Nodes^[OldParent].Children[0] = Sibling then
        begin
  ...
  begin
    if DynamicAABBTree^.Root = Leaf then
    begin
    ...
    begin
      ...
      if DynamicAABBTree^.Nodes^[Parent].Children[0] = Leaf then
      begin
      ...
      begin
        if DynamicAABBTree^.Nodes^[GrandParent].Children[0] = Parent then
        begin
  ...
  function DynamicAABBTreeCreateProxy(const DynamicAABBTree: PDynamicAABBTree;
  const AABB: TAABB; UserData: pointer): longint;
  var
    Node: PDynamicAABBTreeNode;
  begin
  ...
  procedure DynamicAABBTreeDestroyProxy(const DynamicAABBTree: PDynamicAABBTree;
    NodeID: longint);
  begin
  ...
  begin
    ...
    begin
      while assigned(n^.Parent) and (n^.Parent^.Left = n) do
      begin
        n := n^.Parent;
      end;
      n := n^.Parent;
      if assigned(n) and (n <> Root) then
      begin
        Result := n;
      end
      else
  ...
  begin
    ...
    begin
      while assigned(n^.Parent) and (n^.Parent^.Right = n) do
      begin
        n := n^.Parent;
      end;
      n := n^.Parent;
  ...
  begin
    ...
    if assigned(NewParent) and assigned(NewParent^.Right) and
      (NewParent^.Right^.Level = OldParent^.Level) then
    begin
      if OldParent^.Parent^.Left = OldParent then
      begin
        OldParent^.Parent^.Left := NewParent;
  ...
  begin
    ...
    while n <> Root do
    begin
      if (assigned(n^.Left) and (n^.Level <> (n^.Left^.Level + 1))) or
        ((not assigned(n^.Left)) and (n^.Level <> 1)) then
      begin
        VerticalAATreeSkew(Root, n);
        if (not assigned(n^.Right)) or (n^.Level <> n^.Right^.Level) then
        begin
          n := n^.Parent;
        end;
      end;
      if not VerticalAATreeSplit(Root, n^.Parent) then
      begin
        break;
      end;
      n := n^.Parent;
  ...
  function VerticalAATreeFindNode(Root: PVerticalAATreeNode;
    YCoordinate: double): PVerticalAATreeNode;
  var
    n: PVerticalAATreeNode;
  begin
    Result := nil;
    n := Root^.Left;
    while assigned(n) do
    begin
      if YCoordinate = n^.YCoordinate then
      begin
        Result := n;
        break;
      end
      else if YCoordinate < n^.YCoordinate then
      begin
        n := n^.Left;
      end
      else
  ...
  function VerticalAATreeInsertNode(Root: PVerticalAATreeNode;
    YCoordinate: double): PVerticalAATreeNode;
  var
    s: PVerticalAATreeNode;
    LessThan: boolean;
  begin
    ...
    begin
      ...
      s := Root;
      LessThan := True;
      while (LessThan and assigned(s^.Left)) or ((not LessThan) and assigned(s^.Right)) do
      begin
        if LessThan then
        begin
          s := s^.Left;
  ...
  var
    Hash, HashIndex: longword;
  begin
    Hash := HashXY(x, y);
    HashIndex := Hash and HashMask;
    Result := PointHashTable[HashIndex];
    while assigned(Result) and ((Result^.Hash <> Hash) or not
        (ExactSameValue(Result^.x, x) and ExactSameValue(Result^.y, y))) do
    begin
  ...
  begin
    if SameValue(a^.MinY, b^.MinY) then
    begin
      if SameValue(a^.MinX, b^.MinX) then
      begin
        if SameValue(a^.MaxX, b^.MaxX) then
        begin
  ...
  begin
    if EdgeCompare(AtEdge, Edge) <= 0 then
    begin
      Edge^.Next := AtEdge^.Next;
      Edge^.Previous := AtEdge;
      if assigned(AtEdge^.Next) then
      begin
    ...
    begin
      ...
      Edge^.Next := AtEdge;
      if assigned(AtEdge^.Previous) then
      begin
  ...
  begin
    if assigned(Quad^.Previous) then
    begin
      Quad^.Previous^.Next := Quad^.Next;
    end
    else if Quads.First = Quad then
    begin
      Quads.First := Quad^.Next;
    end;
    if assigned(Quad^.Next) then
    begin
      Quad^.Next^.Previous := Quad^.Previous;
    end
    else if Quads.Last = Quad then
    begin
  ...
  begin
    Quad^.AABB.Min.x := Min(Min(Min(Quad^.Points[0]^.x, Quad^.Points[1]^.x),
      Quad^.Points[2]^.x), Quad^.Points[3]^.x);
    Quad^.AABB.Max.x := Max(Max(Max(Quad^.Points[0]^.x, Quad^.Points[1]^.x),
      Quad^.Points[2]^.x), Quad^.Points[3]^.x);
    Quad^.AABB.Min.y := Min(Min(Min(Quad^.Points[0]^.y, Quad^.Points[1]^.y),
      Quad^.Points[2]^.y), Quad^.Points[3]^.y);
    Quad^.AABB.Max.y := Max(Max(Max(Quad^.Points[0]^.y, Quad^.Points[1]^.y),
      Quad^.Points[2]^.y), Quad^.Points[3]^.y);
  end;
  ...
  begin
    ...
    if assigned(Triangles.Last) then
    begin
  ...
  begin
    if assigned(Triangle^.Previous) then
    begin
      Triangle^.Previous^.Next := Triangle^.Next;
    end
    else if Triangles.First = Triangle then
    begin
      Triangles.First := Triangle^.Next;
    end;
    if assigned(Triangle^.Next) then
    begin
      Triangle^.Next^.Previous := Triangle^.Previous;
    end
    else if Triangles.Last = Triangle then
    begin
  ...
  begin
    Triangle^.AABB.Min.x := Min(Min(Triangle^.Points[0]^.x, Triangle^.Points[1]^.x),
      Triangle^.Points[2]^.x);
    Triangle^.AABB.Max.x := Max(Max(Triangle^.Points[0]^.x, Triangle^.Points[1]^.x),
      Triangle^.Points[2]^.x);
    Triangle^.AABB.Min.y := Min(Min(Triangle^.Points[0]^.y, Triangle^.Points[1]^.y),
      Triangle^.Points[2]^.y);
    Triangle^.AABB.Max.y := Max(Max(Triangle^.Points[0]^.y, Triangle^.Points[1]^.y),
      Triangle^.Points[2]^.y);
  end;
  ...
  var
    ...
    InputPolygon: TTriangulationPolygon;
    LastInputPolygonPoint, InputPolygonPoint: PTriangulationPoint;
    Points: array of TTriangulationPoint;
  begin
    Points := nil;
    try
      for i := 0 to length(InputPolygons) - 1 do
      begin
        InputPolygon := InputPolygons[i];
        if length(InputPolygon) > 0 then
        begin
          Count := 0;
          LastInputPolygonPoint := @InputPolygon[length(InputPolygon) - 1];
          if length(Points) < length(InputPolygon) then
          begin
            SetLength(Points, length(InputPolygon) * 2);
          end;
          for j := 0 to length(InputPolygon) - 1 do
          begin
            InputPolygonPoint := @InputPolygon[j];
            if not (SameValue(LastInputPolygonPoint^.x, InputPolygonPoint^.x) and
              SameValue(LastInputPolygonPoint^.y, InputPolygonPoint^.y)) then
            begin
  ...
  var
    PartA, PartB, CurrentEdge: PEdge;
    InSize, PartASize, PartBSize, Merges: longint;
  begin
    if assigned(Edges.First) then
    begin
      InSize := 1;
      while True do
      begin
        ...
        Edges.Last := nil;
        Merges := 0;
        while assigned(PartA) do
        begin
          Inc(Merges);
          PartB := PartA;
          PartASize := 0;
          while PartASize < InSize do
          begin
            Inc(PartASize);
            PartB := PartB^.Next;
            if not assigned(PartB) then
            begin
              break;
            end;
          end;
          PartBSize := InSize;
          while (PartASize > 0) or ((PartBSize > 0) and assigned(PartB)) do
          begin
            if PartASize = 0 then
            begin
              CurrentEdge := PartB;
            ...
            else if (PartBSize = 0) or not assigned(PartB) then
            begin
              CurrentEdge := PartA;
              PartA := PartA^.Next;
              Dec(PartASize);
            end
            else if EdgeCompare(PartA, PartB) <= 0 then
            begin
              CurrentEdge := PartA;
              PartA := PartA^.Next;
              Dec(PartASize);
  ...
  var
    CurrentEdge, WorkEdge, TemporaryEdge, PreviousEdge, NextEdge: PEdge;
  begin
    CurrentEdge := Edges.First;
    if assigned(CurrentEdge) then
    begin
      NextEdge := CurrentEdge^.Next;
      if assigned(NextEdge) then
      begin
        CurrentEdge := NextEdge;
        while assigned(CurrentEdge) do
        begin
          WorkEdge := CurrentEdge;
          TemporaryEdge := WorkEdge^.Previous;
          CurrentEdge := CurrentEdge^.Next;
          if assigned(TemporaryEdge) and (EdgeCompare(TemporaryEdge, WorkEdge) > 0) then
          begin
            repeat
            ...
            else if Edges.Last = WorkEdge then
            begin
              Edges.Last := PreviousEdge;
            end;
            if assigned(TemporaryEdge) then
            begin
  ...
  var
    EdgeA, EdgeB, SplittedEdge, NewEdges: PEdge;
  ...
  begin
    repeat
      NewEdges := nil;
      EdgeA := Edges.First;
      while assigned(EdgeA) do
      begin
        EdgeB := EdgeA^.Next;
        while assigned(EdgeB) and (EdgeB^.MinY <= EdgeA^.MaxY) do
        begin
          if (EdgeA <> EdgeB) and ((EdgeA^.MinX <= EdgeB^.MaxX) and
            (EdgeB^.MinX <= EdgeA^.MaxX)) then
          begin
            a0 := EdgeA^.p;
            a1 := EdgeA^.q;
            b0 := EdgeB^.p;
            ...
            begin
              ...
              if (ai >= 0.0) and (ai <= 1.0) then
              begin
                bi := ((a10x * ab0y) - (a10y * ab0x)) / d;
                if (bi >= 0.0) and (bi <= 1.0) then
                begin
                  d := 1.0 - ai;
                  xi := (a0^.x * d) + (a1^.x * ai);
                  yi := (a0^.y * d) + (a1^.y * ai);
                  if not ((SameValue(a0^.x, xi) and SameValue(a0^.y, yi)) or
                    ...
                    (SameValue(b1^.x, xi) and SameValue(b1^.y, yi))) then
                  begin
  ...
  var
    Current, Start: PVerticalAATreeNode;
  ...
  begin
    repeat
      ...
      begin
        ...
        while assigned(Start) and (Start^.YCoordinate > p0^.y) do
        begin
          Start := VerticalAATreePrevious(@VerticalAATreeNodeRoot, Start);
        end;
        while assigned(Start) and (p0^.y < Start^.YCoordinate) do
        begin
        ...
        repeat
          Current := Start;
          EdgeTryAgain := False;
          while assigned(Current) and (Current^.YCoordinate < p1^.y) do
          begin
            if (p0^.y < p1^.y) and ((p0^.y < Current^.YCoordinate) and
              (Current^.YCoordinate < p1^.y)) and not (ExactSameValue(p0^.y, Current^.YCoordinate) or
              ExactSameValue(p1^.y, Current^.YCoordinate)) then
            begin
              SplittedEdge := EdgeCreate(PointCreate(LerpValue(p0^.x, p1^.x,
                (Current^.YCoordinate - p0^.y) / (p1^.y - p0^.y)), Current^.YCoordinate), Edge^.q, False);
  ...
  procedure SweepVertical;
    ...
    begin
      ...
      CurrentEdge := Edge;
      LastEdge := nil;
      while assigned(CurrentEdge) and (CurrentEdge^.MinY < MaxY) do
      begin
        if (CurrentEdge^.MinY < CurrentEdge^.MaxY) and
          ((CurrentEdge^.MinY < MaxY) and (MinY < CurrentEdge^.MaxY)) then
        begin
          if assigned(LastEdge) then
          begin
            case InputPolygonFillRule of
              ...
              begin
                if LastEdge^.p^.y <= LastEdge^.q^.y then
                begin
                  Inc(Winding);
                end
                else
            ...
            begin
              ...
              if CurrentEdge^.p^.y < CurrentEdge^.q^.y then
              begin
  ...
  var
    Current, Last: PVerticalAATreeNode;
    Edge: PEdge;
  begin
    Edge := Edges.First;
    Last := nil;
    ...
    begin
      ...
      begin
        while assigned(Edge) and (Edge^.MinY < Last^.YCoordinate) do
        begin
          Edge := Edge^.Next;
        end;
  ...
  var
    ax, ay, al, bx, by, bl: double;
  begin
    Result := False;
    ax := a1^.x - a0^.x;
    ay := a1^.y - a0^.y;
    al := sqr(ax) + sqr(ay);
    if not IsZero(al) then
    begin
      bx := b1^.x - b0^.x;
      by := b1^.y - b0^.y;
      bl := sqr(bx) + sqr(by);
      if not IsZero(bl) then
      begin
        al := sqrt(al);
        ax := ax / al;
        ay := ay / al;
  ...
  procedure OptimizeQuads;
    ...
    begin
      Result := False;
      if (PointEquals(Quad.Points[BOTTOMLEFT], OtherQuad.Points[TOPLEFT]) and
        PointEquals(Quad.Points[BOTTOMRIGHT], OtherQuad.Points[TOPRIGHT])) and
        (SameDirection(Quad.Points[TOPLEFT], Quad.Points[BOTTOMLEFT],
        OtherQuad.Points[TOPLEFT], OtherQuad.Points[BOTTOMLEFT]) and
        SameDirection(Quad.Points[TOPRIGHT], Quad.Points[BOTTOMRIGHT],
        OtherQuad.Points[TOPRIGHT], OtherQuad.Points[BOTTOMRIGHT])) then
      begin
        Quad.Points[BOTTOMLEFT] := OtherQuad.Points[BOTTOMLEFT];
        Quad.Points[BOTTOMRIGHT] := OtherQuad.Points[BOTTOMRIGHT];
        Result := True;
      end
      else if (PointEquals(Quad.Points[TOPLEFT], OtherQuad.Points[BOTTOMLEFT]) and
        PointEquals(Quad.Points[TOPRIGHT], OtherQuad.Points[BOTTOMRIGHT])) and
        (SameDirection(Quad.Points[BOTTOMLEFT], Quad.Points[TOPLEFT],
        OtherQuad.Points[BOTTOMLEFT], OtherQuad.Points[TOPLEFT]) and
        SameDirection(Quad.Points[BOTTOMRIGHT], Quad.Points[TOPRIGHT],
        OtherQuad.Points[BOTTOMRIGHT], OtherQuad.Points[TOPRIGHT])) then
      begin
        Quad.Points[TOPLEFT] := OtherQuad.Points[TOPLEFT];
        Quad.Points[TOPRIGHT] := OtherQuad.Points[TOPRIGHT];
        Result := True;
      end
      else if (PointEquals(Quad.Points[TOPRIGHT], OtherQuad.Points[TOPLEFT]) and
        PointEquals(Quad.Points[BOTTOMRIGHT], OtherQuad.Points[BOTTOMLEFT])) and
        (SameDirection(Quad.Points[TOPLEFT], Quad.Points[TOPRIGHT],
        OtherQuad.Points[TOPLEFT], OtherQuad.Points[TOPRIGHT]) and
        SameDirection(Quad.Points[BOTTOMLEFT], Quad.Points[BOTTOMRIGHT],
        OtherQuad.Points[BOTTOMLEFT], OtherQuad.Points[BOTTOMRIGHT])) then
      begin
        Quad.Points[TOPRIGHT] := OtherQuad.Points[TOPRIGHT];
        Quad.Points[BOTTOMRIGHT] := OtherQuad.Points[BOTTOMRIGHT];
        Result := True;
      end
      else if (PointEquals(Quad.Points[TOPLEFT], OtherQuad.Points[TOPRIGHT]) and
        PointEquals(Quad.Points[BOTTOMLEFT], OtherQuad.Points[BOTTOMRIGHT])) and
        (SameDirection(Quad.Points[TOPRIGHT], Quad.Points[TOPLEFT],
        OtherQuad.Points[TOPRIGHT], OtherQuad.Points[TOPLEFT]) and
        SameDirection(Quad.Points[BOTTOMRIGHT], Quad.Points[BOTTOMLEFT],
        OtherQuad.Points[BOTTOMRIGHT], OtherQuad.Points[BOTTOMLEFT])) then
      begin
        Quad.Points[TOPLEFT] := OtherQuad.Points[TOPLEFT];
        Quad.Points[BOTTOMLEFT] := OtherQuad.Points[BOTTOMLEFT];
        Result := True;
      ...
      repeat
        ...
        begin
          ...
          begin
            ...
            if AABBIntersect(Node^.AABB, Quad^.AABB) then
            begin
              if Node^.Children[0] < 0 then
              begin
                OtherQuad := Node^.UserData;
                if Quad <> OtherQuad then
                begin
                  if MergeQuads(Quad, OtherQuad) then
                  begin
              ...
              begin
                if QuadDynamicAABBTree^.StackCapacity <= (LocalStackPointer + 2) then
                begin
                  QuadDynamicAABBTree^.StackCapacity := (LocalStackPointer + 2) * 2;
                  ReallocMem(QuadDynamicAABBTree^.Stack, QuadDynamicAABBTree^.StackCapacity *
                    SizeOf(longint));
                  LocalStack := QuadDynamicAABBTree^.Stack;
                end;
  ...
  procedure TriangulateQuads;
    ...
    begin
      ...
      c := @Quad.Points[2];
      d := @Quad.Points[3];
      if Orientation(a^, b^, c^) < 0.0 then
      begin
        if Orientation(a^, c^, d^) < 0.0 then
        begin
          exit;
        end
        else if Orientation(a^, b^, d^) < 0.0 then
        begin
          Swap(d, c);
        end
        else
      ...
      else if Orientation(a^, c^, d^) < 0.0 then
      begin
        if Orientation(a^, b^, d^) < 0.0 then
        begin
          Swap(b, c);
        end
        else
  ...
  procedure OptimizeTriangles;
    ...
    const
      ModuloNextThree: array[0..2] of longint = (1, 2, 0);
      RemainingVertex: array[0..2, 0..2] of longint = ((-1, 2, 1),
        (2, -1, 0),
        (1, 0, -1));
    var
      i, j, k, l, cb0, cb1, pa, pb, pc, SharedVertices: longint;
      Coincident, Parallel: boolean;
    begin
      Result := False;
      if Triangle <> OtherTriangle then
      begin
        Coincident := False;
        Parallel := False;
        cb0 := -1;
        ...
        begin
          ...
          begin
            l := ModuloNextThree[k];
            if PointEquals(Triangle^.Points[i], OtherTriangle^.Points[k]) then
            begin
              Inc(SharedVertices);
              if SharedVertices > 2 then
              begin
                exit;
              end;
            end;
            if ((PointEquals(Triangle^.Points[i], OtherTriangle^.Points[k]) and
              PointEquals(Triangle^.Points[j], OtherTriangle^.Points[l]))) or
              ((PointEquals(Triangle^.Points[i], OtherTriangle^.Points[l]) and
              PointEquals(Triangle^.Points[j], OtherTriangle^.Points[k]))) then
            begin
              cb0 := k;
              cb1 := l;
              Coincident := True;
            end
            else if PointEquals(Triangle^.Points[j], OtherTriangle^.Points[k]) and
              SameDirection(Triangle^.Points[i], Triangle^.Points[j],
              OtherTriangle^.Points[k], OtherTriangle^.Points[l]) then
            begin
              pa := j;
              pb := k;
              Parallel := True;
            end
            else if PointEquals(Triangle^.Points[j], OtherTriangle^.Points[l]) and
              SameDirection(Triangle^.Points[i], Triangle^.Points[j],
              OtherTriangle^.Points[l], OtherTriangle^.Points[k]) then
            begin
              pa := j;
              pb := l;
              Parallel := True;
            end
            else if PointEquals(Triangle^.Points[i], OtherTriangle^.Points[k]) and
              SameDirection(Triangle^.Points[j], Triangle^.Points[i],
              OtherTriangle^.Points[k], OtherTriangle^.Points[l]) then
            begin
              pa := i;
              pb := k;
            ...
            else if PointEquals(Triangle^.Points[i], OtherTriangle^.Points[l]) and
              SameDirection(Triangle^.Points[j], Triangle^.Points[i],
              OtherTriangle^.Points[l], OtherTriangle^.Points[k]) then
            begin
              pa := i;
        ...
        if (SharedVertices = 2) and Coincident and Parallel then
        begin
          if pb = cb0 then
          begin
  ...
  var
    Triangle, OtherTriangle: PTriangle;
    LocalStack: PDynamicAABBTreeLongintArray;
    LocalStackPointer, NodeID: longint;
    Node: PDynamicAABBTreeNode;
    TryAgain: boolean;
      ...
      repeat
        ...
        begin
          ...
          begin
            ...
            if AABBIntersect(Node^.AABB, Triangle^.AABB) then
            begin
              if Node^.Children[0] < 0 then
              begin
                ...
                if Triangle <> OtherTriangle then
                begin
                  if MergeTriangles(Triangle, OtherTriangle) then
                  begin
              ...
              begin
                if TriangleDynamicAABBTree^.StackCapacity <= (LocalStackPointer + 2) then
                begin
                  TriangleDynamicAABBTree^.StackCapacity := (LocalStackPointer + 2) * 2;
                  ReallocMem(TriangleDynamicAABBTree^.Stack,
                    TriangleDynamicAABBTree^.StackCapacity * SizeOf(longint));
                  LocalStack := TriangleDynamicAABBTree^.Stack;
                end;
                LocalStack^[LocalStackPointer + 0] := Node^.Children[0];
                LocalStack^[LocalStackPointer + 1] := Node^.Children[1];
  ...
  var
    Triangle, Next: PTriangle;
  begin
    Triangle := Triangles.First;
    while assigned(Triangle) do
    begin
      Next := Triangle^.Next;
      OutputTriangle(Triangle^.Points[0]^.x, Triangle^.Points[0]^.y,
  ...
  begin
    ...
    if assigned(PolygonVertex^.Corresponding) and not
      PolygonVertex^.Corresponding.Visited then
    begin
  ...
  function PolygonVertexIsInside(const PolygonVertex: PPolygonVertex;
  const Polygon: PPolygon): boolean;
  var
    Vertex, Next: PPolygonVertex;
    x, y: double;
  begin
    Result := False;
    ...
    y := PolygonVertex^.y;
    repeat
      if (((Vertex^.y < y) and (Next^.y >= y)) or ((Next^.y < y) and (Vertex^.y >= y))) and
        ((Vertex^.x <= x) or (Next^.x <= x)) then
      begin
        Result := Result xor ((Vertex^.x + ((Next^.x - Vertex^.x) *
          ((y - Vertex^.y) / (Next^.y - Vertex^.y)))) < x);
      end;
      Vertex := Vertex^.Next;
      Next := Vertex^.Next;
  ...
  begin
    if assigned(Polygons^.Last) then
    begin
  ...
  var
    Previous, Next: PPolygonVertex;
  begin
    if assigned(Polygon^.First) then
    begin
      Next := Polygon^.First;
  ...
  procedure PolygonInsertVertex(const Polygon: PPolygon;
  const Vertex, Start, Stop: PPolygonVertex);
  var
    Previous, Current: PPolygonVertex;
  begin
    Current := Start;
    while (assigned(Start) and (Current <> Stop)) and (Current^.Distance < Vertex^.Distance) do
    begin
      Current := Current^.Next;
    end;
    Vertex^.Next := Current;
  ...
  function PolygonGetNext(const Polygon: PPolygon;
  const Vertex: PPolygonVertex): PPolygonVertex;
  begin
    Result := Vertex;
    while assigned(Result) and Result^.IsIntersection do
    begin
      Result := Result^.Next;
    end;
  end;
  ...
  begin
    ...
    repeat
      if Result^.IsIntersection and not Result^.Visited then
      begin
        exit;
      end;
      Result := Result^.Next;
  ...
  var
    Vertex: PPolygonVertex;
  begin
    Result := False;
    Vertex := Polygon^.First;
    repeat
      if Vertex^.IsIntersection and not Vertex^.Visited then
      begin
        Result := True;
        exit;
      end;
  ...
  var
    Vertex, Previous, Next: PPolygonVertex;
  begin
    Vertex := Polygon^.First;
    if assigned(Vertex) then
    begin
      Vertex := Vertex^.Next;
      while Vertex <> Polygon^.First do
      begin
        Previous := Vertex^.Previous;
        Next := Vertex^.Next;
        if (Vertex <> Next) and (SameValue(Vertex^.x, Next^.x) and
          SameValue(Vertex^.y, Next^.y)) then
        begin
          Dec(Polygon^.Count);
          if Polygon^.First = Vertex then
          begin
            Polygon^.First := Next;
          end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Next := Vertex^.Next;
        if (Vertex <> Next) and (((SameValue(Vertex^.x, Next^.x) and
          SameValue(Vertex^.y, Next^.y))) or (((Vertex <> Previous) and (Previous <> Next)) and
          SameDirection(Previous, Vertex, Vertex, Next))) then
        begin
          Dec(Polygon^.Count);
          if Polygon^.First = Vertex then
          begin
  ...
  var
    SourceVertex, NextSourceVertex, ClipVertex, NextClipVertex: PPolygonVertex;
  begin
    Result := False;
    ...
    repeat
      ...
      repeat
        ...
        if ((IsZero(Distance(SourceVertex^.x, SourceVertex^.y, ClipVertex^.x,
          ClipVertex^.y, NextClipVertex^.x, NextClipVertex^.y)) and not
          ((SameValue(SourceVertex^.x, ClipVertex^.x) and
          SameValue(SourceVertex^.y, ClipVertex^.y)) or
          (SameValue(SourceVertex^.x, NextClipVertex^.x) and
          SameValue(SourceVertex^.y, NextClipVertex^.y))))) or
          ((IsZero(Distance(ClipVertex^.x, ClipVertex^.y, SourceVertex^.x,
          SourceVertex^.y, NextSourceVertex^.x, NextSourceVertex^.y)) and not
          ((SameValue(ClipVertex^.x, SourceVertex^.x) and
          SameValue(ClipVertex^.y, SourceVertex^.y)) or
          (SameValue(ClipVertex^.x, NextSourceVertex^.x) and
          SameValue(ClipVertex^.y, NextSourceVertex^.y))))) then
        begin
          Result := True;
          exit;
        end
        else if ((SameValue(SourceVertex^.x, ClipVertex^.x) and
          SameValue(SourceVertex^.y, ClipVertex^.y)) and
          (SameValue(NextSourceVertex^.x, NextClipVertex^.x) and
          SameValue(NextSourceVertex^.y, NextClipVertex^.y))) or
          ((SameValue(SourceVertex^.x, NextClipVertex^.x) and
          SameValue(SourceVertex^.y, NextClipVertex^.y)) and
          (SameValue(NextSourceVertex^.x, ClipVertex^.x) and
          SameValue(NextSourceVertex^.y, ClipVertex^.y))) then
        begin
          Result := True;
          exit;
        end;
  ...
  var
    SourceVertex, ClipVertex, s0, s1, c0, c1: PPolygonVertex;
  ...
  begin
    ...
    ClipVertex := Clip^.First;
    repeat
      if not SourceVertex^.IsIntersection then
      begin
        repeat
          if not ClipVertex^.IsIntersection then
          begin
            s0 := SourceVertex;
            ...
            begin
              ...
              if ((ToSource > 0.0) and (ToSource < 1.0)) and ((ToClip > 0.0) and (ToClip < 1.0)) then
              begin
                d := 1.0 - ToSource;
                ...
                if not ((SameValue(IntersectionX, s0^.x) and SameValue(IntersectionY, s0^.y)) or
                  ...
                  (SameValue(IntersectionX, c1^.x) and SameValue(IntersectionY, c1^.y))) then
                begin
                  Result := True;
                  exit;
                end;
  ...
  function PolygonClip(const Source, Clip: PPolygon;
    SourceForwards, ClipForwards: boolean): PPolygons;
  var
    SourceVertex, ClipVertex, SourceIntersection, ClipIntersection, Current,
    s0, s1, c0, c1: PPolygonVertex;
    Clipped: PPolygon;
    s10x, s10y, c10x, c10y, sc0x, sc0y, d, ToSource, ToClip, IntersectionX, IntersectionY: double;
  begin
    Result := nil;
      ...
      begin
        ...
        repeat
          ...
          begin
            ...
            c0 := ClipVertex;
            c1 := PolygonGetNext(Clip, ClipVertex^.Next);
            if (SameValue(s0^.x, c0^.x) and SameValue(s0^.y, c0^.y)) and
              (SameValue(s1^.x, c1^.x) and SameValue(s1^.y, c1^.y)) then
            begin
            ...
            else if (SameValue(s0^.x, c1^.x) and SameValue(s0^.y, c1^.y)) and
              (SameValue(s1^.x, c0^.x) and SameValue(s1^.y, c0^.y)) then
            begin
              PolygonVertexMarkIntersection(s0);
              PolygonVertexMarkIntersection(c0);
              PolygonVertexMarkIntersection(s1);
              ...
              begin
                ...
                begin
                  ...
                  if not ((SameValue(IntersectionX, s0^.x) and SameValue(IntersectionY, s0^.y)) or
                    (SameValue(IntersectionX, s1^.x) and SameValue(IntersectionY, s1^.y)) or
                    (SameValue(IntersectionX, c0^.x) and SameValue(IntersectionY, c0^.y)) or
                    (SameValue(IntersectionX, c1^.x) and
                    SameValue(IntersectionY, c1^.y))) then
                  begin
                    SourceIntersection :=
                      PolygonVertexCreateIntersection(IntersectionX, IntersectionY, ToSource);
                    ClipIntersection := PolygonVertexCreateIntersection(
                      IntersectionX, IntersectionY, ToClip);
                    SourceIntersection^.Corresponding := ClipIntersection;
                    ClipIntersection^.Corresponding := SourceIntersection;
                    PolygonInsertVertex(Source, SourceIntersection, SourceVertex,
                      PolygonGetNext(Source, SourceVertex^.Next));
                    PolygonInsertVertex(Clip, ClipIntersection, ClipVertex,
                      PolygonGetNext(Clip, ClipVertex^.Next));
                  end;
                end
                else if ((IsZero(ToSource) or SameValue(ToSource, 1.0)) and
                  ((ToClip > 0.0) and (ToClip < 1.0))) or
                  (((ToSource > 0.0) and (ToSource < 1.0)) and
                  (IsZero(ToClip) or SameValue(ToClip, 1.0))) then
                begin
                  exit;
                end;
              end;
    ...
    repeat
      if SourceVertex^.IsIntersection then
      begin
    ...
    until SourceVertex = Source^.First;
    repeat
      if ClipVertex^.IsIntersection then
      begin
      ...
      ClipVertex := ClipVertex^.Next;
    until ClipVertex = Clip^.First;
    while PolygonHasUnprocessed(Source) do
    begin
  ...
  begin
    ...
    ca := PolygonClone(a);
    cb := PolygonClone(b);
    if assigned(ca) and assigned(cb) then
    begin
      PolygonClean(ca);
      PolygonClean(cb);
      if PolygonTouching(ca, cb) and not PolygonIntersecting(ca, cb) then
      begin
        Polygons := PolygonUnion(ca, cb);
        if assigned(Polygons) then
        begin
          if assigned(Polygons^.First) and (Polygons^.First = Polygons^.Last) and not
            PolygonSelfIntersecting(Polygons^.First) then
          begin
            PolygonDestroy(a);
            a := Polygons^.First;
            Polygons^.First := nil;
  ...
  begin
    ...
    begin
      ...
      PolygonAddVertex(Quad^.Polygon, PolygonVertexCreate(
    ...
    begin
      ...
      repeat
        ...
        begin
          ...
          begin
            Node := @QuadDynamicAABBTree^.Nodes[NodeID];
            if AABBIntersect(Node^.AABB, AABB) then
            begin
              if Node^.Children[0] < 0 then
              begin
                OtherQuad := Node^.UserData;
                if assigned(OtherQuad) then
                begin
                  OtherPolygon := OtherQuad^.Polygon;
                  if PolygonTryMerge(CurrentPolygon, OtherPolygon) then
                  begin
  ...
  var
    ...
    a, dx0, dy0, dx1, dy1, px, py, vx, vy, nx, ny, vnx, vny, npx, npy, pvx, pvy, cx,
    cy, pcx, pcy, vcx, vcy, ncx, ncy: double;
    IsEar, Convex, FirstSign: boolean;
  begin
    PolygonOptimize(Polygon);
    if Polygon^.Count = 3 then
    begin
      AddTriangle(Polygon^.First^.x, Polygon^.First^.y, Polygon^.First^.Next^.x,
        Polygon^.First^.Next^.y, Polygon^.First^.Next^.Next^.x, Polygon^.First^.Next^.Next^.y);
    end
    else if Polygon^.Count > 3 then
    begin
      Convex := True;
      FirstSign := False;
      ...
      repeat
        ...
        a := (dx0 * dy1) - (dx1 * dy0);
        if PolygonVertex = Polygon^.First then
        begin
          FirstSign := a < 0.0;
        end
        else
        begin
          if FirstSign <> (a < 0.0) then
          begin
            Convex := False;
            break;
          end;
      ...
      begin
        ...
        repeat
          NextPolygonVertex := PolygonVertex^.Next;
          AddTriangle(Polygon^.First^.x, Polygon^.First^.y, PolygonVertex^.x,
            PolygonVertex^.y, NextPolygonVertex^.x, NextPolygonVertex^.y);
          PolygonVertex := NextPolygonVertex;
      ...
      begin
        ...
        try
          ...
          for i := 1 to length(Vertices) - 1 do
          begin
            Vertices[i].ArrayPrevious := @Vertices[i - 1];
          end;
          for i := 0 to length(Vertices) - 2 do
          begin
          ...
          Vertex := Start;
          while Vertex^.Previous <> Vertex^.Next do
          begin
            ...
            begin
              ...
              begin
                ...
                pcx := cx - px;
                pcy := cy - py;
                if (pcx <> 0.0) or (pcy <> 0.0) then
                begin
                  vcx := cx - vx;
                  vcy := cy - vy;
                  if (vcx <> 0.0) or (vcy <> 0.0) then
                  begin
                    ncx := cx - nx;
                    ncy := cy - ny;
                    if (ncx <> 0.0) or (ncy <> 0.0) then
                    begin
                      if (((vnx * vcy) - (vny * vcx)) >= (-EPSILON)) and
                        (((pvx * pcy) - (pvy * pcx)) >= (-EPSILON)) and (((npx * ncy) - (npy * ncx)) >= (-EPSILON)) then
                      begin
                        IsEar := False;
                        break;
                      end;
            ...
            if IsEar then
            begin
              AddTriangle(Vertex^.Previous^.x, Vertex^.Previous^.y, Vertex^.x,
                Vertex^.y, Vertex^.Next^.x, Vertex^.Next^.y);
  ...
  function IsSimple(const Polygon: TTriangulationPolygon): boolean;
  var
    a1i, b1i, k: longint;
    a0p, a1p, b0p, b1p: PTriangulationPoint;
    a10x, a10y, b10x, b10y, ab0x, ab0y, d, ai, bi, xi, yi: double;
  begin
    k := length(Polygon);
    Result := k > 2;
    ...
    begin
      ...
      begin
        a1p := @Polygon[a1i];
        if SameValue(a0p^.x, a1p^.x) and SameValue(a0p^.y, a1p^.y) then
        begin
          Result := False;
          exit;
        end;
              ...
              begin
                ...
                begin
                  ...
                  if not ((SameValue(a0p^.x, xi) and SameValue(a0p^.y, yi)) or
                    ...
                    (SameValue(b1p^.x, xi) and SameValue(b1p^.y, yi))) then
                  begin
                    Result := False;
                    exit;
                  end;
...
begin
  ...
  try
    ...
    begin
      ...
      if (length(InputPolygons) = 1) and (length(InputPolygons[0]) = 3) then
      begin
        OutputTriangle(InputPolygons[0, 0].x, InputPolygons[0, 0].y,
          InputPolygons[0, 1].x, InputPolygons[0, 1].y, InputPolygons[0, 2].x, InputPolygons[0, 2].y);
      end
      else
      begin
        if Quality < 0 then
        ...
        try
          ...
          try
            if (Quality > 3) and ((length(InputPolygons) = 1) and IsSimple(InputPolygons[0])) then
            begin
              Polygon := PolygonCreate;
              for i := 0 to length(InputPolygons[0]) - 1 do
              begin
                PolygonAddVertex(Polygon, PolygonVertexCreate(
                  InputPolygons[0, i].x, InputPolygons[0, i].y));
              end;
              PolygonTriangulate(Polygon);
...
function TriangulateDelaunayClipping(const InputPolygons: TTriangulationPolygons;
  ...
  InputPolygonFillRule: TTriangulationPolygonFillRule = btpfrEVENODD): boolean;
const
  ScaleFactor = 16777216.0;
type
  PDelaunay2DVector = ^TDelaunay2DVector;
...
var
    ...
    begin
      ...
      try
        ...
        begin
          ...
          if IsEar then
          begin
            PushTriangle(Vertex^.Previous^.x, Vertex^.Previous^.y, Vertex^.x,
              Vertex^.y, Vertex^.Next^.x, Vertex^.Next^.y);
            Vertex^.Previous^.Next := Vertex^.Next;
            Vertex^.Next^.Previous := Vertex^.Previous;
            Vertex := Vertex^.Next;
  ...
  function DelaunayTriangulationWatson(const Points: TDelaunay2DVertices;
  var Triangles: TDelaunay2DTriangles): boolean;
  ...
  const
    ...
    dm = dim - 1;
  var
    Vertices: array of array of longint;
  ...
  begin
    ...
    try
      ...
      for i0 := 0 to nrs - 1 do
      begin
        pts[i0, 0] := Points[i0].x;
        pts[i0, 1] := Points[i0].y;
        for i1 := 0 to Dim - 1 do
        begin
          if mxy[0, i1] < pts[i0, i1] then
          begin
            mxy[0, i1] := pts[i0, i1];
          end;
          if mxy[1, i1] > pts[i0, i1] then
          begin
            mxy[1, i1] := pts[i0, i1];
          end;
        end;
    ...
    begin
      ...
      begin
        ...
        try
          ...
          if DelaunayTriangulationWatson(Delaunay2DVertices, Delaunay2DTriangles) then
          begin
            Clipper := TClipper.Create;
            try
              for i := 0 to length(Delaunay2DTriangles) - 1 do
              begin
                ...
                for j := 0 to length(TempPaths) - 1 do
                begin
...
begin
  ...
  try
    ...
    begin
      ...
      if (length(InputPolygons) = 1) and (length(InputPolygons[0]) = 3) then
      begin
        PushTriangle(InputPolygons[0, 0].x, InputPolygons[0, 0].y, InputPolygons[0, 1].x,
          InputPolygons[0, 1].y, InputPolygons[0, 2].x, InputPolygons[0, 2].y);
      end
      else
      begin
        ...
        begin
          ...
          for j := 0 to length(InputPolygons[i]) - 1 do
          begin
...
var
  ...
  begin
    ...
    Result^.p := p;
    Result^.q := q;
    if IsInfinite(p^.x) or IsInfinite(p^.y) or IsInfinite(q^.x) or
      IsInfinite(q^.y) or IsZero(q^.x - p^.x) then
    begin
  ...
  begin
    ...
    begin
      ...
      else if Polygon^.Last = Node then
      begin
  ...
  procedure PolygonInsertBefore(const Polygon: PPolygon; const Point: PPoint;
  const Node: PPolygonNode);
  var
    NewNode: PPolygonNode;
  begin
  ...
  var
    ConvexPoints: array of PPolygonNode;
    ...
    begin
      if (not Node^.Ear) and (Node <> Polygon^.First) and (Node <> Polygon^.Last) and
        ((Cross(Node^.Next^.Point, Node^.Previous^.Point, Node^.Point) > 0.0) = Positive) then
      begin
        Node^.Ear := True;
        Inc(CountConvexPoints);
        if length(ConvexPoints) < CountConvexPoints then
        begin
    ...
    try
      ...
      begin
        ...
        begin
          ...
          while p <> Polygon^.Last do
          begin
            AddEar(p);
            p := p^.Next;
          end;
          ConvexPointIndex := 0;
          while ConvexPointIndex < CountConvexPoints do
          begin
  ...
  function NewTrapezoid(const LeftPoint, RightPoint: PPoint;
  const Top, Bottom: PEdge): PTrapezoid;
  begin
  ...
  begin
    if assigned(Trapezoid^.Sink) then
    begin
  ...
  function QueryGraphLocate(const QueryGraph: PQueryGraph; const Point: PPoint;
  const Slope: double): PTrapezoid;
  var
    Node: PNode;
    Orient: double;
  begin
    ...
    begin
      if assigned(Node^.Trapezoid) then
      begin
      ...
      begin
        if assigned(Node^.Point) then
        begin
          if Point^.x >= Node^.Point^.x then
          begin
            Node := Node^.Right;
          end
          else
        ...
        else if assigned(Node^.Edge) then
        begin
          ...
          else if Slope < Node^.Edge^.Slope then
          begin
            Node := Node^.Right;
          end
          else
  ...
  procedure QueryGraphCase1(const QueryGraph: PQueryGraph; const Sink: PNode;
  const Edge: PEdge; const t1, t2, t3, t4: PTrapezoid);
  var
    yNode, qNode: PNode;
  begin
  ...
  procedure QueryGraphCase2(const QueryGraph: PQueryGraph; const Sink: PNode;
  const Edge: PEdge; const t1, t2, t3: PTrapezoid);
  var
    yNode: PNode;
  begin
    yNode := SetYNode(NewNode, Edge, GetSink(t2), GetSink(t3));
  ...
  procedure QueryGraphCase3(const QueryGraph: PQueryGraph; const Sink: PNode;
  const Edge: PEdge; const t1, t2: PTrapezoid);
  begin
    SetYNode(Sink, Edge, GetSink(t1), GetSink(t2));
  end;

  procedure QueryGraphCase4(const QueryGraph: PQueryGraph; const Sink: PNode;
  const Edge: PEdge; const t1, t2, t3: PTrapezoid);
  var
    yNode: PNode;
  begin
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if not t^.Inside then
        begin
          t^.Inside := True;
          if assigned(t^.UpperLeft) then
          begin
            if length(Stack) <= StackPointer then
            begin
  ...
  procedure TrapezoidAddPoint(const Trapezoid: PTrapezoid; const Edge: PEdge;
  const Point: PPoint);
  var
    Polygon: PPolygon;
    v: PPolygonNode;
  begin
    ...
    begin
      ...
      while assigned(v) do
      begin
        if not neg(Point, v^.Point) then
        begin
          exit;
        end;
        if Point^.x < v^.Point^.x then
        begin
    ...
    begin
      if neg(Point, Edge^.p) and neg(Point, Edge^.q) then
      begin
        Polygon := NewPolygon;
  ...
  procedure TrapezoidalMapPush(const TrapezoidalMap: PTrapezoidalMap;
  const Trapezoid: PTrapezoid);
  begin
    if assigned(TrapezoidalMap^.Last) then
    begin
  ...
  function TrapezoidalMapNextTrapezoid(const TrapezoidalMap: PTrapezoidalMap;
  const t: PTrapezoid; const Edge: PEdge): PTrapezoid;
  begin
    if Edge^.q^.x <= t^.RightPoint^.x then
    begin
      Result := FalseTrapezoid;
    end
    else if EdgeAbove(Edge, t^.RightPoint) then
    begin
      Result := t^.UpperRight;
    end
    else
  ...
  function TrapezoidalMapCase1(const TrapezoidalMap: PTrapezoidalMap;
  const t: PTrapezoid; const e: PEdge): PTrapezoid;
  var
    t2, t3, t4: PTrapezoid;
    Sink: PNode;
  begin
  ...
  function TrapezoidalMapCase2(const TrapezoidalMap: PTrapezoidalMap;
  const t: PTrapezoid; const e: PEdge): PTrapezoid;
  var
    Next, t2, t3: PTrapezoid;
    Sink: PNode;
  begin
  ...
  function TrapezoidalMapCase3(const TrapezoidalMap: PTrapezoidalMap;
  const t: PTrapezoid; const e: PEdge): PTrapezoid;
  var
    Next, t1, t2, LowerRight, LowerLeft: PTrapezoid;
    Bottom, Top: PEdge;
    Sink: PNode;
  begin
    Next := TrapezoidalMapNextTrapezoid(TrapezoidalMap, t, e);
    ...
    Top := t^.Top;
    if TrapezoidalMap^.tcross = t^.Top then
    begin
      t1 := t^.UpperLeft;
    ...
    begin
      ...
      if assigned(e^.Above) then
      begin
        e^.Above^.LowerRight := t1;
      end;
      t1^.LowerRight := nil;
    end;
    if TrapezoidalMap^.bcross = Bottom then
    begin
      t2 := LowerLeft;
  ...
  function TrapezoidalMapCase4(const TrapezoidalMap: PTrapezoidalMap;
  const t: PTrapezoid; const e: PEdge): PTrapezoid;
  var
    Next, t1, t2: PTrapezoid;
    Sink: PNode;
  begin
    Next := TrapezoidalMapNextTrapezoid(TrapezoidalMap, t, e);
    if TrapezoidalMap^.tcross = t^.Top then
    ...
    if TrapezoidalMap^.bcross = t^.Bottom then
    begin
      t2 := t^.LowerLeft;
      t2^.RightPoint := e^.q;
    end
  ...
  function TrapezoidalMapAddEdge(const TrapezoidalMap: PTrapezoidalMap;
  const Edge: PEdge): boolean;
  var
    t: PTrapezoid;
    cp, cq: boolean;
  begin
    ...
    begin
      cp := False;
      cq := False;
      while assigned(t) and (t <> FalseTrapezoid) do
      begin
        if cp then
        begin
          cp := False;
        ...
        else if cp and not cq then
        begin
          t := TrapezoidalMapCase2(TrapezoidalMap, t, Edge);
        end
        else if (not cp) and (not cq) then
        begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (Current^.Top = TrapezoidalMap^.Root^.Top) and
          (assigned(Current^.Bottom^.Below) and not Current^.Bottom^.Below^.Removed) then
        begin
          TrapezoidMarkInside(Current^.Bottom^.Below);
          break;
        end;
        if (Current^.Bottom = TrapezoidalMap^.Root^.Bottom) and
          (assigned(Current^.Top^.Above) and not Current^.Top^.Above^.Removed) then
        begin
    ...
    while assigned(Current) do
    begin
      if Current^.Inside and not Current^.Removed then
      begin
        if IsInfinite(Current^.Top^.p^.y) then
        begin
          exit;
        end;
  ...
  function ShearTransform(const p: TTriangulationPoint): PPoint;
  const
    SHEAR = 1e-10;
  begin
  ...
  try
    ...
    try
      ...
      begin
        ...
        for i := 0 to h - 1 do
        begin
          if not TrapezoidalMapAddEdge(Map, Edges[i]) then
          begin
            exit;
          end;
        end;
        if not TrapezoidalMapCollectPoints(Map) then
        begin
          exit;
        end;
        CountOutputTriangles := 0;
        for i := 0 to h - 1 do
        begin
          if assigned(Edges[i]^.Polygon) and (Edges[i]^.Polygon^.Length > 0) then
          begin
...
begin
  CountOutputTriangles := 0;
  if length(InputPolygon) = 3 then
  begin
    PushTriangle(InputPolygon[0].x, InputPolygon[0].y, InputPolygon[1].x,
      InputPolygon[1].y, InputPolygon[2].x, InputPolygon[2].y);
  end
  else if length(InputPolygon) > 3 then
  begin
    Convex := True;
    FirstSign := False;
    for i := 0 to length(InputPolygon) - 1 do
    begin
      j := i + 1;
      if j >= length(InputPolygon) then
      begin
        j := 0;
      end;
      k := j + 1;
      if k >= length(InputPolygon) then
      begin
        k := 0;
      end;
    ...
    begin
      ...
      for i := 1 to length(InputPolygon) - 2 do
      begin
        PushTriangle(InputPolygon[0].x, InputPolygon[0].y, InputPolygon[i].x,
          InputPolygon[i].y, InputPolygon[i + 1].x, InputPolygon[i + 1].y);
      end;
    end
    else
    begin
